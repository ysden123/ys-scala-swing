...
object Dialogs extends SimpleSwingApplication:
  ...
  private lazy val label = new Label("No Result yet")
  private lazy val tabs = new TabbedPane {
    ...
    pages += new Page("Simple Modal Dialogs", new BorderPanel {
      ...
      val radios: Seq[RadioButton] = List(ok, ynlf, ynp, yncp)
    ...
    pages += new Page("More Dialogs", new BorderPanel {
      ...
      val radios: Seq[RadioButton] = List(pick, enter, custom, customUndec, custom2)
      mutex.buttons ++= radios
      mutex.select(pick)
      val buttons: BoxPanel = new BoxPanel(Orientation.Vertical) {
        contents ++= radios
      ...
      layout(new Button(Action("Show It!") {
        ...
        mutex.selected.get match {
          case `pick` =>
            ...
            label.text = if (s.isDefined && s.get.nonEmpty)
