...
object TableSelection extends SimpleSwingApplication:
  private val model = Array[Array[Any]](List("Mary", "Campione", "Snowboarding", 5, false).toArray,
  ...
  lazy val ui: BoxPanel = new BoxPanel(Orientation.Vertical) {
    ...
    val multiInterval: RadioButton = radio(intervalMutex, "Multiple Interval Selection")
    val elementInterval: RadioButton = radio(intervalMutex, "Single Selection")
    val singleInterval: RadioButton = radio(intervalMutex, "Single Interval Selection")
    intervalMutex.select(multiInterval)

    contents += new Label("Selection Options")
    val elemMutex = new ButtonGroup
    val rowSelection: RadioButton = radio(elemMutex, "Row Selection")
    val columnSelection: RadioButton = radio(elemMutex, "Column Selection")
    val cellSelection: RadioButton = radio(elemMutex, "Cell Selection")
